...
for i = 1:length(pI)
    k.KA  = pI(i);
    % Correcting effect of large KA by adjusting gA:
%     k.gA = k.gA*10;
    ...
    % ODE
    save('Par_ODE2.mat','k');
    [t,y] = ode45(@FN_ODE_PID2,[0 max(k.P(1,:))+2000],abs([Z1 Z2 X1 XC A M]));
...
for i = 1:length(pI)
    k.KA  = pI(i);
    hold on;
    plot(Y(i).t,k.KA./Y(i).y(:,5),'LineWidth',1,...
        'Color',C(i*floor(64/length(pI)),:),...
        'DisplayName',num2str(k.KA))
end
    ylabel('K_A/A ','FontSize',14)
    xlabel('Minutes','FontSize',14)
    box('on')
    set(gca,'FontSize',14,'XGrid','on')
    xlim([750 5000])
